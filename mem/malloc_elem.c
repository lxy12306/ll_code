...
#define ll_memset(x,y,z) memset(x,y,z)
...
malloc_elem_init(struct malloc_elem *elem, struct malloc_heap *heap,
		struct ll_memseg_list *msl, size_t size,
		struct malloc_elem *orig_elem, size_t orig_size, bool dirty) {
	elem->heap = heap;
	elem->msl = msl;
	elem->prev = NULL;
	elem->next = NULL;
	ll_memset(&elem->free_list, 0, sizeof(elem->free_list));
...
/*
 * Insert the malloc_elem to his heap
 */
void
malloc_elem_insert(struct malloc_elem *elem) {
	...
	if ((heap->first == NULL) != (heap->last == NULL)) {
		LL_LOG(ERR, EAL, "Heap is probably corrupt\n");
		return;
	}

	if (heap->first == NULL) {
		//if heap is empty
		heap->first = heap->last = elem;
		prev_elem = next_elem = NULL;
		...
		dist_from_end = LL_PTR_DIFF(heap->last, elem);
		dist_from_start = LL_PTR_DIFF(elem, heap->first);
 ...
 */
static bool
elem_check_phys_contig(const struct ll_memseg_list *msl,
 ...
 */
static void *
elem_start_pt(struct malloc_elem *elem, size_t size, unsigned align, size_t bound, bool contig) {
	...
	while (elem_size >= size) {
		...
		uintptr_t new_data_start = LL_ALIGN_FLOOR((end_pt - size), align);
		uintptr_t new_elem_start;
		
		//check boundary
		
		if ((new_data_start & bmask) != ((end_pt - 1) & bmask)) {
			end_pt = LL_ALIGN_FLOOR(end_pt, bound);
			new_data_start = LL_ALIGN_FLOOR((end_pt - size), align);
		...
		if (new_data_start < (uintptr_t)elem) {
			return NULL;
		}
...
split_elem(struct malloc_elem *elem, struct malloc_elem *split_pt) {
	...
	const size_t old_elem_size = LL_PTR_DIFF(elem, split_pt);
	...
	set_trailer(elem);
	if (elem->pad && elem->state != ELEM_PAD) {
		/* Update inner padding inner element size. */
		elem = LL_PTR_ADD(elem, elem->pad);
...
next_elem_is_adjacent(struct malloc_elem *elem)
{
	...
	return elem->next == LL_PTR_ADD(elem, elem->size) &&
...
prev_elem_is_adjacent(struct malloc_elem *elem)
{
	...
	return elem == LL_PTR_ADD(elem->prev, elem->prev->size) &&
...
#define MALLOC_LOG2_INCREMENT 2
	...
	return index <= LL_HEAP_NUM_FREELISTS - 1 ?
			index : LL_HEAP_NUM_FREELISTS - 1;
}
...
malloc_elem_free_list_insert(struct malloc_elem *elem)
{
	...
	idx = malloc_elem_free_list_index(elem->size - MALLOC_ELEM_HEADER_LEN - MALLOC_ELEM_TRAILER_LEN);
	elem->state = ELEM_FREE;
...
malloc_elem_alloc(struct malloc_elem *elem, size_t size, unsigned align,
	...
	const size_t old_elem_size = LL_PTR_DIFF(new_elem, elem);
	...
	if (trailer_size > MALLOC_ELEM_OVERHEAD + MIN_DATA_SIZE) {
		...
		struct malloc_elem *new_free_elem = LL_PTR_ADD(new_elem, size + MALLOC_ELEM_OVERHEAD);
...
join_elem(struct malloc_elem *elem1, struct malloc_elem *elem2) {
	...
	if (elem1->pad) {
		struct malloc_elem *inner = LL_PTR_ADD(elem1, elem1->pad);
...
malloc_elem_join_adjacent_free(struct malloc_elem *elem) {
	...
	if (elem->next != NULL && elem->next->state == ELEM_FREE && next_elem_is_adjacent(elem)) {
		...
		erase = LL_PTR_SUB(elem->next, MALLOC_ELEM_HEADER_LEN);
		...
		ll_memset(erase, MALLOC_POISON, erase_len);
	}
	...
	if (elem->prev != NULL && elem->prev->state == ELEM_FREE && prev_elem_is_adjacent(elem)) {
		...
		erase = LL_PTR_SUB(elem, MALLOC_ELEM_TRAILER_LEN);
...
malloc_elem_free(struct malloc_elem *elem)
{
	...
	ptr = LL_PTR_ADD(elem, MALLOC_ELEM_HEADER_LEN);
	...
	if (!elem->dirty)
		ll_memset(ptr, 0, data_len);
#else
	/* Always poison the memory in debug mode. */
	ll_memset(ptr, MALLOC_POISON, data_len);
#endif
...
malloc_elem_hide_region(struct malloc_elem *elem, void *start, size_t len) {
	...
	if (next && next_elem_is_adjacent(elem)) {
		len_after = LL_PTR_DIFF(hide_end, next);
		if (len_after >= MALLOC_ELEM_OVERHEAD + MIN_DATA_SIZE) {
			...
			LL_LOG(ERR, EAL, "Unaligned element, heap is probably corrupt\n");
			return;
		}
	}
	...
	if (prev && prev_elem_is_adjacent(elem)) {
		len_before = LL_PTR_DIFF(hide_start, elem);
...
malloc_elem_resize(struct malloc_elem *elem, size_t size) {
	...
	//check if there is a next element, it's fress and adjacent
	if (!elem->next || elem->next->state !=  ELEM_FREE || !next_elem_is_adjacent(elem)) {
		return -1;
	}
	
	if (elem->next->size < new_size - elem->size) {
		return -1;
	}
	...
	if (elem->size - new_size >= MALLOC_ELEM_OVERHEAD + MIN_DATA_SIZE) {
		/* now we have a big block together. Lets cut it down a bit, by splitting */
		struct malloc_elem *split_pt = LL_PTR_ADD(elem, new_size);
		split_pt = LL_PTR_ALIGN_CEIL(split_pt, LL_CACHE_LINE_SIZE);
